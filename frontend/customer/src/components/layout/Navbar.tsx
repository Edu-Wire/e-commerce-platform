...
export default function Navbar() {
  ...
  const handleImageSearch = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/products/search-by-image`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.success) {
        ...
        toast.error(data.error || 'Could not find matching products');
      }
    } catch (err) {
      toast.dismiss(toastId);
          ...
          <form onSubmit={handleSearch} className="w-full flex h-full group">
            <div ref={searchRef} className="relative flex w-full h-full bg-white rounded-md ring-offset-0 focus-within:ring-2 focus-within:ring-[#febd69]">
              ...
              <button
                ...
                className="px-2 text-gray-500 hover:text-orange-500 transition-colors flex items-center justify-center"
